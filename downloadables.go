...
func newDownloadables(sdkConfig sdkConfiguration) *Downloadables {
...
func (s *Downloadables) Get(ctx context.Context, token string, opts ...operations.Option) (*operations.CustomerPortalDownloadablesCustomerPortalDownloadablesGetResponse, error) {
	...
	if retryConfig != nil {
		...
		if err != nil || httpRes == nil {
			...
			_, err = s.sdkConfiguration.Hooks.AfterError(hooks.AfterErrorContext{HookContext: hookCtx}, nil, err)
			return nil, err
		} else if utils.MatchStatusCodes([]string{"400", "404", "410", "422", "4XX", "5XX"}, httpRes.StatusCode) {
			_httpRes, err := s.sdkConfiguration.Hooks.AfterError(hooks.AfterErrorContext{HookContext: hookCtx}, httpRes, nil)
			if err != nil {
				return nil, err
			} else if _httpRes != nil {
	...
	res := &operations.CustomerPortalDownloadablesCustomerPortalDownloadablesGetResponse{
		HTTPMeta: components.HTTPMetadata{
			Request:  req,
			Response: httpRes,
		},
